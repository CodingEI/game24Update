...
const login = async (req, res) => {
  ...
  try {
    const [rows] = await connection.query(
      "SELECT * FROM users WHERE (dial_code = ? AND phone = ?) OR email = ?",
      [dialcode, username, username],
    );
...
const register = async (req, res) => {
  try {
    ...
    let { full_name, currency, email, username, pwd, invitecode, countryCode } =
      req.body;
    ...
    let real_money = process.env.REAL_MONEY_ON_REGISTER || 0;
    ...
    if (check_u.length > 0) {
      return res.status(200).json({
        message: "Registered phone number or Email address.",
        status: false,
      });
    }
    ...
    const sql =
      "INSERT INTO users SET id_user = ?, dial_code = ?, email = ?, full_name = ?, currency = ? ,phone = ?,name_user = ?,password = ?,plain_password = ?, money = ?,bonus_money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
    ...
    await connection.query(sql4, [username, code, invitecode || null]);
