...
// };
const listMember = async (req, res) => {
  const { search = "" } = req.body; // Only search parameter is needed

  // Base SQL query
  let sql = `
    SELECT * FROM users 
    WHERE veri = 1 AND level != 2
    ${search ? "AND (phone LIKE ? OR id_user LIKE ?)" : ""}
    ORDER BY id DESC
  `;
  ...
  try {
    ...
    const [users] = await connection.execute(
      `SELECT * FROM users WHERE veri = 1 AND level != 2 ORDER BY id DESC LIMIT 10`,
      [],
    );
    ...
    return res.status(200).json({
      message: "Success",
      status: true,
      datas: users, // Return all users without pagination
    });
  } catch (error) {
...
const updateQrcodeImage = async (req, res, next) => {
  ...
  const { bkash_mobile, nagad_mobile } = req.body;
  ...
  await connection.execute(
    "UPDATE admin_ac SET bkash_mobile = ? , nagad_mobile = ? WHERE id = 1",
    [bkash_mobile, nagad_mobile],
  );
