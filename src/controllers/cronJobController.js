...
import newTrxWinGoController from "./newTrxWingoController.js";

const cronJobGame1p = (io) => {
  // execute in every 30 secs
  ...
  cron.schedule("*/1 * * * *", async () => {
    await newTrxWinGoController.addNewTrxWinGo(1);
    await newTrxWinGoController.handlingNewTrxWinGo1P(1);
    const [trxWingo] = await connection.execute(
      `SELECT * FROM new_trx_wingo WHERE game = '${TRX_WINGO_GAME_TYPE_MAP.MIN_1}' ORDER BY id DESC LIMIT 2`,
      [],
    );
  ...
  //   await newTrxWinGoController.addNewTrxWinGo(3);
  //   await newTrxWinGoController.handlingNewTrxWinGo1P(3);
  //   const [trxWingo] = await connection.execute(
  //     `SELECT * FROM new_trx_wingo WHERE game = '${TRX_WINGO_GAME_TYPE_MAP.MIN_3}' ORDER BY id DESC LIMIT 2`,
  //     [],
  //   );
  //   io.emit("data-server-trx-wingo", { data: trxWingo });
  ...
  //   await newTrxWinGoController.addNewTrxWinGo(5);
  //   await newTrxWinGoController.handlingNewTrxWinGo1P(5);
  //   const [trxWingo] = await connection.execute(
  //     `SELECT * FROM new_trx_wingo WHERE game = '${TRX_WINGO_GAME_TYPE_MAP.MIN_5}' ORDER BY id DESC LIMIT 2`,
  //     [],
  //   );
  //   io.emit("data-server-trx-wingo", { data: trxWingo });
  ...
  //   await newTrxWinGoController.addNewTrxWinGo(10);
  //   await newTrxWinGoController.handlingNewTrxWinGo1P(10);
  //   const [trxWingo] = await connection.execute(
  //     `SELECT * FROM new_trx_wingo WHERE game = '${TRX_WINGO_GAME_TYPE_MAP.MIN_10}' ORDER BY id DESC LIMIT 2`,
  //     [],
  //   );
  //   io.emit("data-server-trx-wingo", { data: trxWingo });
