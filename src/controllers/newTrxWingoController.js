...
const newTrxWinGoPage = async (req, res) => {
  return res.render("bet/newTrxWingo/win.ejs");
};

const newTrxWinGoPage3 = async (req, res) => {
  return res.render("bet/newTrxWingo/win3.ejs");
};

const newTrxWinGoPage5 = async (req, res) => {
  return res.render("bet/newTrxWingo/win5.ejs");
};

const newTrxWinGoPage10 = async (req, res) => {
  return res.render("bet/newTrxWingo/win10.ejs");
};
...
const distributeCommission = async () => {
  try {
    ...
    // const [bets] = await connection.query('SELECT phone, SUM(money + fee) AS total_money FROM new_trx_wingo_bets WHERE time > ? AND time <= ? GROUP BY phone', [startOfDay, endTime]);
...
//   const [bets] = await connection.query('SELECT phone,SUM(money) AS total_money FROM new_trx_wingo_bets GROUP BY phone');
...
const betNewTrxWinGo = async (req, res) => {
  try {
    ...
    const [winGoNow] = await connection.query(
      "SELECT period FROM new_trx_wingo WHERE status = 0 AND game = ? ORDER BY id DESC LIMIT 1",
      [gameJoin],
    );
    ...
    if (check >= 0) {
      const sql = `INSERT INTO new_trx_wingo_bets SET 
        id_product = ?,
        phone = ?,
        code = ?,
        invite = ?,
...
const listOrderOld = async (req, res) => {
  ...
  const [wingo] = await connection.query(
    "SELECT * FROM new_trx_wingo WHERE status != 0 AND game = ? ORDER BY id DESC LIMIT ?, ?",
    [game, Number(pageno), Number(pageto)],
  );
  const [wingoAll] = await connection.query(
    "SELECT COUNT(*) as game_length FROM new_trx_wingo WHERE status != 0 AND game = ?",
    [game],
  );
...
const GetMyEmerdList = async (req, res) => {
  ...
  const [new_trx_wingo_bets] = await connection.query(
    "SELECT * FROM new_trx_wingo_bets WHERE phone = ? AND game = ? ORDER BY id DESC LIMIT ?, ?",
    [user[0].phone, game, Number(pageno), Number(pageto)],
  );
  const [new_trx_wingo_betsAll] = await connection.query(
    "SELECT COUNT(*) as bet_length FROM new_trx_wingo_bets WHERE phone = ? AND game = ?",
    [user[0].phone, game],
  );

  if (!new_trx_wingo_bets[0]) {
    return res.status(200).json({
      code: 0,
      msg: "No more data",
      data: {
  ...
  if (!pageno || !pageto || !user[0] || !new_trx_wingo_bets[0]) {
    return res.status(200).json({
      message: "Error!",
      status: true,
    });
  }
  let page = Math.ceil(new_trx_wingo_betsAll[0].bet_length / 10);

  let datas = new_trx_wingo_bets.map((data) => {
...
const addNewTrxWinGo = async (game) => {
  try {
    let join = "";
    ...
    const [winGoNow] = await connection.query(
      "SELECT period , block_id FROM new_trx_wingo WHERE status = 0 AND game = ? ORDER BY id DESC LIMIT 1",
      [join],
    );
    ...
    if (isPendingGame) {
      ...
      const [minPlayers] = await connection.query(
        "SELECT * FROM new_trx_wingo_bets WHERE status = 0 AND game = ?",
        [join],
      );
      ...
      let BlockId, BlockTime, Hash;

      if (winGoNow.length != 0) {
        BlockId = parseInt(winGoNow[0]?.block_id || 6541232) + 1;
      } else {
        BlockId = 6541232;
      }
      ...
      await connection.query(
        "UPDATE new_trx_wingo  SET block_id = ?, block_time = ?, hash = ? WHERE period = ? AND game = ?",
        [BlockId, BlockTime, Hash, amount, previousPeriod, join],
      );
      ...
      if (nextResult == "-1") {
        ...
        await connection.query(
          "UPDATE new_trx_wingo  SET amount = ?, status = 1, release_status = 1 WHERE period = ? AND game = ?",
      ...
      let adminWingoKey = "";
      if (game == 1) adminWingoKey = "trx_wingo1";
      if (game == 3) adminWingoKey = "trx_wingo3";
      if (game == 5) adminWingoKey = "trx_wingo5";
      if (game == 10) adminWingoKey = "trx_wingo10";
    ...
    let NewGamePeriod = await generateWingoPeriodId(join);

    let NewBlockId, NewBlockTime, NewHash;

    if (winGoNow.length != 0) {
      NewBlockId = parseInt(winGoNow[0]?.block_id || 6541232) + 1;
    } else {
      NewBlockId = 6541232;
    }

    NewBlockTime = Date.now();
    NewHash = uuidv4();

    await connection.query(
      `
         INSERT INTO new_trx_wingo
         SET block_id = ?, block_time = ?, hash = ?, period = ?, amount = 0, game = ?, status = 0, time = ?
      `,
      [NewBlockId, NewBlockTime, NewHash, NewGamePeriod, join, timeNow],
    );
  } catch (error) {
    if (error) {
      console.log(error);
...
const handlingNewTrxWinGo1P = async (typeid) => {
  try {
    let game = "";
    if (typeid == 1) game = "trx_wingo";
    if (typeid == 3) game = "trx_wingo3";
    ...
    const [winGoNow] = await connection.query(
      "SELECT * FROM new_trx_wingo WHERE status = 1 AND release_status = 1 AND game = ? ORDER BY id DESC LIMIT 1",
      [game],
    );
    ...
    await connection.query(
      "UPDATE new_trx_wingo_bets SET result = ? WHERE status = 0 AND game = ?",
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      await connection.query(
        "UPDATE `new_trx_wingo_bets` SET `get` = ?, `status` = 1 WHERE `id` = ?",
    ...
    await connection.query(
      "UPDATE new_trx_wingo SET release_status = 2 WHERE period = ? AND game = ?",
...
const batchUpdateBetStatus = async (result, game) => {
  ...
  while (true) {
    const [rows] = await connection.execute(
      `UPDATE new_trx_wingo_bets SET status = 2 
...
export async function generateWingoPeriodId(game) {
  const [res] = await connection.execute(
    "SELECT * FROM new_trx_wingo WHERE game = ? ORDER BY id DESC LIMIT 1",
    [game],
  );
  if (res.length === 0) {
    // let gameRepresentationId = GameRepresentationIds.WINGO[game];
    ...
    return parseInt(res[0].period) + 1;
  }
}

const newTrxWinGoController = {
  newTrxWinGoPage,
  betNewTrxWinGo,
  listOrderOld,
  GetMyEmerdList,
  handlingNewTrxWinGo1P,
  addNewTrxWinGo,
...
export default newTrxWinGoController;
