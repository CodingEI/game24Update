...
async function funHanding(game) {
  ...
  const betCategories = {
    big_even: ["12", "14", "16", "18", "b"],
    big_odd: ["11", "13", "15", "17", "b"],
    small_even: ["4", "6", "8", "10", "s"],
    small_odd: ["3", "5", "7", "9", "s"],
  };

  // xử lý game Tổng số
  const resultTotalBetCatgories = await Promise.all(
    Object.keys(betCategories).map(async (key) => {
      const [allbet] = await connection.execute(
        `SELECT SUM(money) as total FROM result_k3 WHERE status = 0 AND game = ${game} AND typeGame = 'total'`,
      );
      return {
        name: key,
        total_bet: parseFloat(allbet[0]?.total || 0),
      };
    }),
  );

  const smallesTotalCategory = resultTotalBetCatgories.reduce(
    (smallest, category) =>
      !smallest || category.total_bet < smallest.total_bet
        ? category
        : smallest,
  );

  totalResult =
    betCategories[smallesTotalCategory.name][
      Math.floor(
        Math.random() * betCategories[smallesTotalCategory.name].length,
      )
    ];

  console.log({ resultTotalBetCatgories, totalResult });
  await connection.execute(
    `UPDATE result_k3 SET status = 2 WHERE game = ${game} AND typeGame = 'total' AND bet NOT IN ${betCategories[smallesTotalCategory.name].map((_) => "?")} ) `,
    [...betCategories[smallesTotalCategory.name]],
  );

  // xử lý game 2 số trùng nhau

  const resultTwoSameBetCatgories = await Promise.all(
    Object.keys(betCategories).map(async (key) => {
      const [allbet] = await connection.execute(
        `SELECT SUM(money) as total FROM result_k3 WHERE status = 0 AND game = ${game} AND typeGame = 'two-same'`,
      );
      return {
        name: key,
        total_bet: parseFloat(allbet[0]?.total || 0),
  ...
  const smallesTwosameCategory = resultTwoSameBetCatgories.reduce(
    (smallest, category) =>
      !smallest || category.total_bet < smallest.total_bet
        ? category
        : smallest,
  ...
  twoSameResult =
    betCategories[smallesTwosameCategory.name][
      Math.floor(
        Math.random() * betCategories[smallesTwosameCategory.name].length,
      )
    ];

  await connection.execute(
    `UPDATE result_k3 SET status = 2 WHERE game = ${game} AND typeGame = 'two-same' AND bet NOT IN ${betCategories[smallesTwosameCategory.name].map((_) => "?")} ) `,
    [...betCategories[smallesTwosameCategory.name]],
  );

  // xử lý game 3 số trùng nhau
  const resultThreeSameBetCatgories = await Promise.all(
    Object.keys(betCategories).map(async (key) => {
      const [allbet] = await connection.execute(
        `SELECT SUM(money) as total FROM result_k3 WHERE status = 0 AND game = ${game} AND typeGame = 'three-same'`,
      );
      return {
        name: key,
        total_bet: parseFloat(allbet[0]?.total || 0),
  ...
  const smallesThreeSameCategory = resultThreeSameBetCatgories.reduce(
    (smallest, category) =>
      !smallest || category.total_bet < smallest.total_bet
        ? category
        : smallest,
  ...
  threeSameResult =
    betCategories[smallesThreeSameCategory.name][
      Math.floor(
        Math.random() * betCategories[smallesThreeSameCategory.name].length,
      )
    ];

  await connection.execute(
    `UPDATE result_k3 SET status = 2 WHERE game = ${game} AND typeGame = 'three-same' AND bet NOT IN ${betCategories[smallesThreeSameCategory.name].map((_) => "?")} ) `,
    [...betCategories[smallesThreeSameCategory.name]],
  );

  // xử lý game 3 số khác nhau
  const resultUnlikeBetCatgories = await Promise.all(
    Object.keys(betCategories).map(async (key) => {
      const [allbet] = await connection.execute(
        `SELECT SUM(money) as total FROM result_k3 WHERE status = 0 AND game = ${game} AND typeGame = 'unlike'`,
      );
      return {
        name: key,
        total_bet: parseFloat(allbet[0]?.total || 0),
  ...
  const smallesUnlikeSameCategory = resultUnlikeBetCatgories.reduce(
    (smallest, category) =>
      !smallest || category.total_bet < smallest.total_bet
        ? category
        : smallest,
  ...
  unlikeResult =
    betCategories[smallesUnlikeSameCategory.name][
      Math.floor(
        Math.random() * betCategories[smallesUnlikeSameCategory.name].length,
      )
    ];

  await connection.execute(
    `UPDATE result_k3 SET status = 2 WHERE game = ${game} AND typeGame = 'unlike' AND bet NOT IN ${betCategories[smallesUnlikeSameCategory.name].map((_) => "?")} ) `,
    [...betCategories[smallesUnlikeSameCategory.name]],
  );
  ...
  await connection.execute(
    ...
    [`${totalResult}${twoSameResult}${threeSameResult}`],
  );
}
