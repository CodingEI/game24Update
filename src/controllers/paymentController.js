...
const PaymentMethodsMap = {
  ...
  BDT_MANUAL_1: "bdt_manual_1",
  BDT_MANUAL_2: "bdt_manual_2",
  WOW_PAY: "wow_pay",
  RS_PAY: "rs_pay",
  USDT: "usdt",
  UPAY: "upay",
...
const addManualUSDTPaymentRequest = async (req, res) => {
  try {
    ...
    let money = money_usdt * process.env.USDT_INR_EXCHANGE_RATE_FOR_DEPOSIT;
    let utr = parseInt(data.utr);
...
// BDT Payment Integration
const initiateManualBDTPayment = async (req, res) => {
  const query = req.query;

  // Bkash Or Nagad
  const platform = query.platform;

  if (platform !== "bkash" && platform !== "nagad") {
    return;
  }
  ...
  return res.render("wallet/bdt_manual_payment.ejs", {
    Amount: query?.am,
    scan_image:
      query.platform === "bkash" ? setting.bkash_scan : setting.nagad_scan,
    mobile:
      query.platform === "bkash" ? setting.bkash_mobile : setting.nagad_mobile,
    type: platform,
  });
};

const addManualBDTPaymentRequest = async (req, res) => {
  try {
    const data = req.body;
    let auth = req.cookies.auth;
    let money_bdt = parseInt(data.money);
    const platformType = req.body.platform;
    let money = parseFloat(money_bdt);
    let utr = data.utr;
    const minimumMoneyAllowed = parseInt(process.env.MINIMUM_MONEY_BDT);

    console.log({ money, minimumMoneyAllowed });

    if (!money || money <= minimumMoneyAllowed) {
      return res.status(400).json({
        message: `Money is Required and it should be BDT ${minimumMoneyAllowed.toFixed(2)} or above!`,
        status: false,
        timeStamp: timeNow,
      });
    }
    ...
    const pendingRechargeList = await rechargeTable.getRecordByPhoneAndStatus({
      ...
      type:
        platformType === "bkash"
          ? PaymentMethodsMap.BDT_MANUAL_1
          : PaymentMethodsMap.BDT_MANUAL_2,
    });
...
const browseRechargeRecord = async (req, res) => {
  try {
    ...
    const [recharge] = await connection.query(
      `SELECT * FROM recharge WHERE status = 0 AND (type = '${PaymentMethodsMap.UPI_MANUAL}' OR type = '${PaymentMethodsMap.USDT_MANUAL}' OR type =  '${PaymentMethodsMap.BDT_MANUAL_1}' OR type = '${PaymentMethodsMap.BDT_MANUAL_2}')`,
      [],
    );
