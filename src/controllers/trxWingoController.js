  ...
  try {
    ...
    if (isPendingGame) {
      ...
      if (isAdminManipulatedResult) {
        ...
        const [oldTrxWingo] = await connection.query(
          "SELECT period, block_id,  block_time FROM trx_wingo_game WHERE status = 1 AND game = ? ORDER BY id DESC LIMIT 1",
          [join],
        );

        let BlockId, BlockTime, Hash;
        if (oldTrxWingo.length != 0) {
          BlockId = parseInt(oldTrxWingo[0]?.block_id) + 1;
        } else {
          BlockId = 6541232;
        }
    ...
    let NewGamePeriod = await generateTrxWingoPeriodId(join);

    console.log({ NewGamePeriod });
...
const handlingTrxWingo1P = async (typeid) => {
  try {
    ...
    await connection.query(
      `UPDATE trx_wingo_bets SET status = 2 WHERE status = 0 AND game = ? AND bet NOT IN (${validBets.map(() => "?").join(",")})`,
      [game, ...validBets],
    );
...
export async function generateTrxWingoPeriodId(game) {
  const [res] = await connection.execute(
    "SELECT * FROM trx_wingo_game WHERE game = ? ORDER BY id DESC LIMIT 1",
    [game],
  );
