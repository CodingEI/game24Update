...
const uploadsDir = path.join(path.resolve(), "src", "public", "uploads");
...
const verifyCode = async (req, res) => {
  ...
  if (user.time_otp - now <= 0) {
    request(
      `https://otp.indiahost.org/send_otp2.php?mobile=+91${phone}&otp=1234&user=bada123&key=bada123`,
...
const infoUserBank = async (req, res) => {
  ...
  try {
    ...
    const [lastRecharge] = await connection.query(
      `SELECT * FROM recharge WHERE phone = '${userInfo.phone}' AND status = 1 ORDER BY time DESC LIMIT 1`,
    );
    ...
    const [lastBetResetTime] = await connection.query(
      `SELECT time FROM recharge WHERE phone = '${userInfo.phone}' AND status = 1 AND remaining_bet = 0 ORDER BY time DESC LIMIT 1`,
    );
    const betTimeInterval =
      lastBetResetTime.length === 0
        ? ""
        : `AND time > ${lastBetResetTime[0].time}`;
    ...
    if (calculation < 0 && lastRecharge.length > 0) {
      await connection.query(
        `UPDATE recharge SET remaining_bet = 0, time = ? WHERE id = ? AND status = 1`,
...
const transfer = async (req, res) => {
  ...
  let dial_code = req.body.dial_code;
  ...
  //  check if user has transferred money already for today
  const [transfers] = await connection.execute(
    "SELECT * FROM balance_transfer WHERE sender_phone = ? AND DATE(datetime)  = CURDATE()",
    [sender_phone],
  );

  if (transfers.length >= 1) {
    return res.status(200).json({
      message: "You can transfer a maximum of once per day.",
      status: false,
      timeStamp: timeNow,
    });
  }

  const transferLimitPerDay = parseFloat(
    process.env.MAXIMUM_WALLET_TRANSFER_PER_DAY,
  );

  if (parseFloat(amount) > transferLimitPerDay) {
    return res.status(200).json({
      message: `"You can transfer up to ${transferLimitPerDay} per day."`,
      status: false,
      timeStamp: timeNow,
    });
  }
  ...
  if (sender_money >= amount) {
    let [receiver] = await connection.query(
      "SELECT * FROM users WHERE dial_code = ? AND `phone` = ? LIMIT 1",
      [dial_code, receiver_phone],
    );
...
const listUsedRedenvelops = async (req, res) => {
  let auth = req.cookies.auth;
  const [[user]] = await connection.execute(
    "SELECT * FROM users WHERE token = ?",
    [auth],
  );
  let [redenvelopes] = await connection.execute(
    "SELECT * FROM redenvelopes_used WHERE phone_used = ? ORDER BY time DESC LIMIT 30",
    [user.phone],
  );
...
const createTicket = async (req, res) => {
  ...
  try {
    for (let i = 0; i < req.files.length; i++) {
      ...
      const imageName =
        new Date().valueOf() + path.extname(req.files[i].originalname);
    ...
    await connection.execute(
      "INSERT INTO tickets (`phone`, `title`, `description`, `images`, `status`) VALUES ( ?, ? ,? ,? ,?)",
      [user.phone, subject, description, JSON.stringify(imagePaths), 0],
    );
...
const getTicketHistory = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ success: true, message: "Tickets fetched.", data: tickets });
};
