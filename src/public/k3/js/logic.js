...
const displayResultHandler = ({ status, amount, period, result }) => {
  ...
  const sum = resultSplit[2]
    .split("")
    .reduce((prev, total) => parseInt(prev) + parseInt(total));
  ...
  const resultHtml = `
    ...
    <div data-v-2d418cc5="" class="line2">
        ...
        <div data-v-2d418cc5="" class="yuan">${resultSplit[0] == "b" ? "Big" : "Small"}</div>
        <div data-v-2d418cc5="">${resultSplit[1] == "l" ? "Even" : "Odd"}</div>
    </div>
   `;
    ...
    .map((list_order, index) => {
      ...
      return `
        ...
        <div data-v-a5ef3154="" class="MyGameRecordList__C-detail details_box_${index}" style="display: none">
          ...
          <div data-v-a5ef3154="" class="MyGameRecordList__C-detail-line">
              Win/lose 
              <div data-v-a5ef3154="" class="${list_order.status == 1 ? "green" : "red"}">${list_order.status == 1 ? `${list_order.get.toFixed(2)}` : `- ${list_order.price}`}</div>
          </div>
          <div data-v-a5ef3154="" class="MyGameRecordList__C-detail-line">
              Order time 
...
async function RenderResult(results) {
  ...
  let result = String(results).split(",")[2].split("");

  // Ensure result[0] is greater than 0
  if (Number(result[0]) <= 0) {
    result[0] = "1"; // Set to 1 if it's 0 or negative
  }

  $(".K3TL__C-l3 .box div:eq(0)").attr("class", `num${result[0]}`);
  $(".K3TL__C-l3 .box div:eq(1)").attr("class", `num${result[1] ?? 5}`);
  $(".K3TL__C-l3 .box div:eq(2)").attr("class", `num${result[2] ?? 2}`);
  return false;
}
...
socket.on("data-server-k3", async function (msg) {
  ...
  try {
    ...
    const lastGameBets = betListData?.filter(
      (game) => Number(game.stage) === Number(lastGame.period),
    );

    console.log("lastGameBets", lastGameBets);

    const lostGames = lastGameBets?.filter((game) => game.status === 2);
    ...
    if (lastGameBets.length > 0) {
      if (winGamesMoney > 0) {
        console.log("calll 1111111111111111111");
        displayResultHandler({
          status: STATUS_MAP.WIN,
        ...
        console.log("calll 222222222222222222222222");
        displayResultHandler({
          status: STATUS_MAP.LOSS,
